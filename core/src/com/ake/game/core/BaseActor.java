...
public class BaseActor extends Actor
{
    ...
    public void flipAnimation(boolean status){
        this.flip = status;
    }
    ...
    public Vector2 isWithinDistance2(float distance, BaseActor other) {
        Polygon poly1 = this.getBoundaryPolygon();
        float scaleX = (this.getWidth() + 2 * distance) / this.getWidth();
        float scaleY = (this.getHeight() + 2 * distance) / this.getHeight();
        poly1.setScale(scaleX, scaleY);
        ...
        this.moveBy(mtv.normal.x * mtv.depth * 2, mtv.normal.y * mtv.depth * 2);
        return mtv.normal;
    }
    ...
    public static ArrayList<BaseActor> getList(Stage stage, String className)
    {
        ...
        for (Actor a : stage.getActors())
        {
            try {
                ...
                System.out.println("NullPointerException is caught be warned");
            }
        }
    ...
    /**
     * Gets angle between actor and mouse (-PI  PI)
     */
    public float getMouseAngle(){
        Vector3 mouse = stage.getCamera().unproject(new Vector3(Gdx.input.getX(), Gdx.input.getY(), 0));
        Vector3 difference = mouse.sub(this.getX() + this.getOriginX(), this.getY() + this.getOriginY(), 0f);
        difference.nor();

        float angle = MathUtils.atan2(difference.y, difference.x) * MathUtils.radiansToDegrees;

        return(MathUtils.map(-180f, 179f, 0f, 359f, angle));
    }
    /**
    ...
    public void draw(Batch batch, float parentAlpha) 
    {
        ...
        if ( animation != null && isVisible() )
            batch.draw( animation.getKeyFrame(elapsedTime), getX(), getY(), getOriginX(), getOriginY(), getWidth(), getHeight(), (this.flip ? -1 : 1) * getScaleX(), getScaleY(), getRotation() );
